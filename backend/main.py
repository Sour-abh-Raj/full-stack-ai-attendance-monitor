...
import cv2
from fastapi import FastAPI, File, UploadFile, Form, UploadFile, Response, HTTPException
from fastapi.responses import FileResponse
from fastapi.middleware.cors import CORSMiddleware
import face_recognition
import starlette
from vicinity_checker import check_location_in_vicinity

VISCINITY_ACCURACY = 200
PATH_TO_GEO_TAG = "./current_location_tag.csv"
...
async def login(file: UploadFile = File(...)):
    if not check_location_in_vicinity(VISCINITY_ACCURACY, PATH_TO_GEO_TAG):
        raise HTTPException(status_code=400, detail="Error: Vicinity Check Failed!")
